...
def doGMM(pca_dimensions):
    #raw vectorized image
    if pca_dimensions==0:
        train_x_reduced = train_x
        test_x_reduced = test_x
    else:
        n_pca = PCA(n_components=pca_dimensions)
        n_pca.fit(train_x)
    ...
    if pca_dimensions==0:
        ...
        plt.title('{} dimensions'.format(pca_dimensions))
    plt.scatter(test_x_reduced[:,0], test_x_reduced[:,1], c=predictions)
    plt.plot
